...
uint16_t modbus_crc16(const uint8_t *data, uint16_t len)
{
    ...
    while (len--) {
        /* CRC计算：当前CRC低8位与新字节异或作为查表索引 */
        index = (uint8_t)(crc ^ *data++);
        /* 新CRC = (旧CRC右移8位) 异或 查表值 */
...
int modbus_crc_verify(const uint8_t *data, uint16_t len)
{
    ...
    if (len < 2) {
        return -1;  /* 数据太短，无法包含CRC */
    }

    /* 计算CRC（不含最后2字节） */
    calculated_crc = modbus_crc16(data, len - 2);

    /* 提取接收到的CRC（小端序：低字节在前） */
    received_crc = (uint16_t)data[len - 1] << 8 | data[len - 2];
...
void modbus_crc_append(uint8_t *data, uint16_t len)
{
    ...
    /* Modbus RTU使用小端序：低字节在前，高字节在后 */
    data[len]     = (uint8_t)(crc & 0xFF);        /* CRC低字节 */
    data[len + 1] = (uint8_t)((crc >> 8) & 0xFF); /* CRC高字节 */
}
